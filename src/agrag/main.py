...
def get_defaults_from_config():
    ...
    try:
        with open(DATA_PROCESSING_MODULE_CONFIG, "r") as f:
            ...
            DATA_PROCESSING_MODULE_DEFAULTS = {
                k: v if v else doc["data"][k] for k, v in DATA_PROCESSING_MODULE_DEFAULTS.items()
            }
    except FileNotFoundError:
        print(f"Error: File not found - {DATA_PROCESSING_MODULE_CONFIG}")
    except yaml.YAMLError as exc:
        print(f"Error parsing YAML file - {DATA_PROCESSING_MODULE_CONFIG}: {exc}")
    except Exception as exc:
        print(f"Unexpected error occurred while loading {DATA_PROCESSING_MODULE_CONFIG}: {exc}")
    ...
    try:
        ...
        print(f"Error parsing YAML file - {EMBEDDING_MODULE_CONFIG}: {exc}")
    except Exception as exc:
        print(f"Unexpected error occurred while loading {EMBEDDING_MODULE_CONFIG}: {exc}")
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        metavar="",
        required=False,
        default=EMBEDDING_MODULE_DEFAULTS["DEFAULT_EMBEDDING_MODEL"],
    )
    parser.add_argument(
        "--pooling_strategy",
        type=str,
        help="Strategy to use when pooling the embeddings generated by the embedding model. Options are 'mean', 'max', 'cls' (default is None).",
        metavar="",
        required=False,
        default=None,
    )
