...
from ragify.constants import EVALUATION_DIR, EVALUATION_MAX_FILE_SIZE
from ragify.evaluation.utils import (
...
class EvaluationModule:
    ...
    def get_queries_and_responses(self, query_fn: Callable, response_fn: Callable):
        ...
        for idx, row in enumerate(self.dataset):
            ...
            generated_response = self.ragify.generate_response(query)
    ...
    def run_evaluation(
        ...
        if not self.ragify.pipeline_initialized:
            self.ragify.initialize_rag_pipeline()
        else:
            logger.info("\nRAGify pipeline already initialized...Skipping")
